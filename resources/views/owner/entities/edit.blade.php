...
@section('title', 'Редакция на обект')
    ...
    <div class="mx-auto max-w-5xl space-y-6 bg-white">
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-gray-300 p-4">
            ...
            <div class="flex flex-wrap gap-2">
                <a href="{{ route('owner.entities.show', $entity) }}" class="rounded border border-gray-400 px-3 py-2 text-sm text-black hover:bg-gray-100">
                    Назад към прегледа
                </a>
                <a href="{{ route('owner.entities.media.edit', $entity) }}" class="rounded border border-gray-400 px-3 py-2 text-sm text-black hover:bg-gray-100">
                    Медия
                </a>
            </div>
        </div>
        ...
        <form method="POST" action="{{ route('owner.entities.update', $entity) }}" class="space-y-4 rounded-lg border border-gray-300 p-4">
            @csrf
            @method('PUT')
...
@push('scripts')
    <script>
        $(function () {
            ...
            var searchUrl = @json(route('owner.places.search'));
